...
namespace CSharp设计模式
{
    ...
    //实际应用场景：
    //1、数据库操作：在数据库应用中，访问者模式可以用来访问数据库中的不同数据对象，并进行不同的操作，例如查询、插入、更新、删除等。
    //2、UI控件：在图形用户界面应用中，访问者模式可以用来访问不同的UI控件，并进行不同的操作，例如渲染、事件处理、布局等。
    //3、编译器：在编译器应用中，访问者模式可以用来访问语法树中的不同节点，并进行不同的操作，例如语法检查、语义分析、代码生成等。
    //4、数学运算：在数学应用中，访问者模式可以用来访问不同的数学对象，并进行不同的运算，例如加、减、乘、除等。
